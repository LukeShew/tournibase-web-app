...
import { recordGateSaleForScanner } from "@/app/scan/[scanner-token]/actions";
import { GateSaleForm } from "@/components/gate-sale-form";
import { ScannerUnavailable } from "@/components/scanner-unavailable";
import { getGateSaleTicketOptions } from "@/lib/gate-sales";
...
export default async function GateSalePage({
  params,
}: {
  ...
  try {
    ...
    console.error("[gate-sale] session lookup failed", {
  ...
  if (!lookup.session.permissions.includes("manual_sale")) {
    return <ScannerUnavailable status="permission" />;
  }

  let tickets: Awaited<ReturnType<typeof getGateSaleTicketOptions>>;

  try {
    tickets = await getGateSaleTicketOptions(
      lookup.session.tournamentId,
    );
  } catch (error) {
    console.error("[gate-sale] ticket lookup failed", {
      message: error instanceof Error ? error.message : "Unknown error",
    });
  ...
  const recordSale = recordGateSaleForScanner.bind(null, scannerToken);
