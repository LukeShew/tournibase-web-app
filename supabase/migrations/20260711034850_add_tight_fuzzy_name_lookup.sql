-- Allow a very small typo in buyer-name lookup while keeping identifiers exact.

create extension if not exists fuzzystrmatch with schema extensions;

create or replace function public.matches_tight_name(
  p_value text,
  p_query text
)
returns boolean
...
as $$
  ...
  select
    ...
    and not exists (
      ...
      where not exists (
        select 1
        from value_tokens as value_token
        where position(query_token.token in value_token.token) > 0
          or position(value_token.token in query_token.token) > 0
          or (
            char_length(query_token.token) >= 4
            and abs(
              char_length(query_token.token) - char_length(value_token.token)
            ) <= case when char_length(query_token.token) >= 10 then 2 else 1 end
          ...
          or (
            char_length(query_token.token) >= 4
            and char_length(query_token.token) = char_length(value_token.token)
            and exists (
              select 1
              from generate_series(
                1,
                char_length(query_token.token) - 1
              ) as swap(position)
              where overlay(
                overlay(
                  query_token.token
                  placing substr(query_token.token, swap.position + 1, 1)
                  from swap.position for 1
                )
                placing substr(query_token.token, swap.position, 1)
                from swap.position + 1 for 1
              ) = value_token.token
            )
          )
      )
...
revoke all on function public.matches_tight_name(text, text)
  from public, anon, authenticated;
grant execute on function public.matches_tight_name(text, text)
  to service_role;
  ...
  with matching_orders as (
    ...
    where customer_order.tournament_id = v_scanner.tournament_id
      ...
      and (
        ...
        or public.matches_tight_name(customer_order.buyer_name, v_query)
